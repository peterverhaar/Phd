...
void visualisePoem( String poem ) {
  ...
  String title = poem.substring( 71 , ( poem.length() - 4 ) );
  ...
  for (TableRow row : table.rows()) {
    ...
    for ( int w = 0 ; w < codes.length ; w++ ) {
      ...
      noStroke();
        ...
        //text ( cs , indent + align - (w2+1)*35    , Y  ) ; 

      if( c == 0 ) {
        fill(#B8B8B8);                        
        rect( indent + align - (w2+1)*35    , Y , width , (height -4  ) ) ; 

      } else {
        fill( cols[c] );        
        rect( indent + align - (w2+1)*35    , Y , width , (height  ) ) ; 
