...
void draw( ) {
  ...
  {
    text( files[i].toString() , 30 , i*20 ) ;  
    
  
    if ( files[i].toString().toLowerCase().endsWith(".csv") ) {
      visualisePoem( files[i].toString() ) ; 
    }
...
void visualisePoem( String poem ) {
  
    println( poem ) ;  
 
  background(249);  
  
  table = loadTable( poem , "header");
  
  fill(0);
  textSize(26); 
  

  String title = poem.substring( 72 , ( poem.length() - 4 ) );



  
  int nrRows = table.getRowCount() ;
  size( 600 , 50*nrRows ) ; 

  text( title , 20 , 20 ) ;
  
    //text(table.getRowCount() + " total rows in table", 10, 20 ); 
  ...
  for (TableRow row : table.rows()) {
    ...
    int Nr = row.getInt("line");
    
     
    if ( Nr > 0 ) {
    
    String rhyme = row.getString("rhyme");
    String alliteration = row.getString("alliteration");
    ...
    for ( int w = 0 ; w < codes.length ; w++ ) {
      ...
      if( c == 0 ) {
        ...
        rect( indent + (w+1)*35 , Y , width , ( height ) ) ; 
    ...
    String allit[] = new String[] { };
    allit = split( alliteration , " " );
    

    for ( int w = 0 ; w < allit.length ; w++ ) {
